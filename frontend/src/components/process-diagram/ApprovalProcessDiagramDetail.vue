<!-- src/components/process-diagram/ApprovalProcessDiagramDetail.vue -->
<template>
  <div class="diagram-detail-container">
    <div class="header">
      <h2>{{ isEditMode ? '编辑审批流程图' : '新增审批流程图' }}</h2>
...
export default {
  ...
  methods: {
    ...
    async loadAllDiagrams() {
      try {
        this.allDiagrams = await processDiagramService.getAllApprovalProcessDiagrams();
        // 确保数据存在且为数组
        if (Array.isArray(this.allDiagrams)) {
          // 限制初始显示数量
        ...
        console.error('加载审批流程图列表失败:', error);
        // 不中断用户操作，只是不显示拷贝功能
        this.allDiagrams = [];
        this.filteredDiagrams = [];
      }
    ...
    async selectDiagramFromDropdown(diagram) {
      ...
      try {
        // 获取选中的流程图详情
        const diagramDetail = await processDiagramService.getApprovalProcessDiagramById(diagram.id);
    ...
    copyDiagramData(diagramData) {
      ...
      // 特别确保imageDataUrl存在
      if (diagramData.imageDataUrl && !this.form.imageDataUrl) {
        this.form.imageDataUrl = diagramData.imageDataUrl;
      }
    ...
    async handleSubmit() {
      ...
      try {
        ...
        if (this.isEditMode) {
          // 编辑模式
          response = await processDiagramService.updateApprovalProcessDiagram(this.form.id, formData);
        } else {
          // 新增模式
          response = await processDiagramService.createApprovalProcessDiagram(formData);
        }

        this.$emit('diagram-updated', response);
        alert(this.isEditMode ? '审批流程图更新成功' : '审批流程图创建成功');
      } catch (error) {
    ...
    openApiUrl() {
      if (this.form.id) {
        // 构造API URL，假设API端点为 /api/process-diagrams/approval/{id}
        const apiUrl = `${window.location.origin}/api/process-diagrams/approval/${this.form.id}`;
