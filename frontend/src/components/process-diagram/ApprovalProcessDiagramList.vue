...
export default {
  ...
  methods: {
    ...
    async activateDiagram(diagramId) {
      try {
        await processDiagramService.activateApprovalProcessDiagram(diagramId);
        await this.fetchDiagrams();
      } catch (error) {
        console.error('激活审批流程图失败:', error);
        alert('激活审批流程图失败: ' + (error.message || '未知错误'));
      }
    },

    async deactivateDiagram(diagramId) {
      try {
        await processDiagramService.deactivateApprovalProcessDiagram(diagramId);
        await this.fetchDiagrams();
      } catch (error) {
        console.error('停用审批流程图失败:', error);
        alert('停用审批流程图失败: ' + (error.message || '未知错误'));
      }
    },
    ...
    async deleteDiagram(diagramId) {
      ...
      try {
        await processDiagramService.deleteApprovalProcessDiagram(diagramId);
        await this.fetchDiagrams();
      } catch (error) {
        console.error('删除审批流程图失败:', error);
        alert('删除审批流程图失败: ' + (error.message || '未知错误'));
      }
    },
