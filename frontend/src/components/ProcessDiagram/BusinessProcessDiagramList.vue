  ...
  methods: {
    ...
    async saveDiagram() {
      try {
        ...
        // 新增流程图
        await processDiagramService.createBusinessProcessDiagram(formData);
        await this.fetchDiagrams();
        this.closeForm();
        alert('业务流程图创建成功');
      } catch (error) {
    ...
    async toggleDiagramStatus(id, isValid) {
      try {
        ...
        if (isValid) {
          // 启用流程图
          await processDiagramService.activateBusinessProcessDiagram(id);
        } else {
          // 禁用流程图
          await processDiagramService.deactivateBusinessProcessDiagram(id);
        }
    ...
    async deleteDiagram(id) {
      ...
      try {
        await processDiagramService.deleteBusinessProcessDiagram(id);
        await this.fetchDiagrams();
        // 如果正在查看被删除的流程图，则返回列表
